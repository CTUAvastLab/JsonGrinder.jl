...
For such cases, we have introduced additional `Schema`, a `MultiEntry,` but we discourage to rely on this feature and recommend to adapt JSONs to have stable schema (if possible).
This can be achieved by modifying each sample before it's passed into the schema.

Each subtype of `Schema` implements the `update!` function, which recursively updates the schema.

### Entry
```julia
mutable struct Entry{T} <: Schema
	counts::Dict{T,Int}
	updated::Int
end
```
...
```
`ArrayEntry` keeps information about arrays (e.g. `"a": [1,2,3,4]`). Statistics about individual items of the array are deferred to `item`, which can be `<:Schema`. `l` keeps histogram of lengths of arrays, and `updated` is number of times an array has been observed in particular place in JSON.
...
Fields `childs` contains all keys which were observed in specific Dictionary and their corresponding `<:Schema` values with statistics about values observed under given key.

### MultiEntry
```julia
mutable struct MultiEntry <: Schema
	childs::Vector{Schema}
	updated::Int
end
```
